<div class="bg-white shadow-xl p-4 w-96">
    ...
    <form method="POST" action="{{ $formActionURL }}" autocomplete="off">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="name" class="w-full border p-1 focus:outline-none focus:border-blue-800" value="{{ $data['user']['name'] ?? '' }}" required autofocus />
        </div>
        <div class="mb-3">
            <label class="block text-sm text-gray-600 mb-1">E-Mail</label>
            <input type="email" name="email" class="w-full border p-1 focus:outline-none focus:border-blue-800" value="{{ $data['user']['email'] ?? '' }}" required />
        </div>
        <div class="mb-4">
            ...
            <input type="password" name="password" class="w-full border p-1 focus:outline-none focus:border-blue-800" {{ isset($data) ? '':'required' }} />
        </div>
